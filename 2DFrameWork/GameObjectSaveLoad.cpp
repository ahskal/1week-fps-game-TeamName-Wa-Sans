...
void GameObject::SaveObject(Xml::XMLElement* This, Xml::XMLDocument* doc)
{
	...
	if (color) {
		Xml::XMLElement* Color = doc->NewElement("Shader_Color");
		This->LinkEndChild(Color);
		Color->SetAttribute("R", color.x);
		Color->SetAttribute("G", color.y);
		Color->SetAttribute("B", color.z);
		Color->SetAttribute("A", color.w);
	}
	if (collider)
	{
		//Collider
...
void GameObject::LoadObject(Xml::XMLElement* This)
{
	...
	if (component = This->FirstChildElement("Shader_Color"))
	{
		color.x = component->FloatAttribute("R");
		color.y = component->FloatAttribute("G");
		color.z = component->FloatAttribute("B");
		color.w = component->FloatAttribute("A");
	}
